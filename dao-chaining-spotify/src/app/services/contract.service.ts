...
export class ContractService {
  ...
  async createOrganization(orgID, payableWallet, orgName, tokenAddress) {
    ...
    this.uDonate = new this.web3js.eth.Contract(
      chaining_spotify_abi,
      chaining_spotify_address
    );
  ...
  async isOrganizationAdmin(orgID) {
    ...
    const checkAdmin = await this.uDonate.methods
      .isOrgAdmin(orgID, this.accounts[0])
  ...
  async donate(id, amount, tip) {
    ...
    const donate = await this.uDonate.methods
      .donateETH(id, tip)
      .send({ from: this.accounts[0], value: updatedAmt });
    return donate;
  }
  ...
  async pauseOrganization(id, causeIds) {
    ...
    const pause = await this.uDonate.methods
  ...
  async unpauseOrganization(id, causeIds) {
    ...
    const unpause = await this.uDonate.methods
  ...
  async addNewOrganizationAdmin(address, id) {
    ...
    const add = await this.uDonate.methods
      .addAdmin(address, id)
      .send({ from: this.accounts[0] });
    console.log(add);
  }
  ...
  async removeOrganizationAdmin(address, id) {
    ...
    const remove = await this.uDonate.methods
      .removeAdmin(address, id)
      .send({ from: this.accounts[0] });
    console.log(remove);
  }
}
